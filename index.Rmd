...
Genome-wide SNP datasets provide several advantages over microsatellite markers for population and conservation genomic studies. Whereas microsatellite analyses typically rely on a relatively small number of highly polymorphic loci [@guichoux2011], target-capture sequencing datasets can have thousands of SNPs distributed across hundreds of loci [@grover2012], substantially increasing statistical power for estimating genetic diversity, detecting fine-scale population structure, and inferring demographic history and gene flow. These expanded datasets also enable analyses that are difficult or impossible with microsatellite datasets, including tests of historical gene flow using approaches such as TreeMix and f-statistics, providing novel information for these complex systems. 

Although reduced-representation approaches such as Restriction-site-Associated DNA sequencing (RADseq) generate genome-wide SNPs as well, target-capture offers several practical advantages. These included a more consistent recovery of homologous loci across samples, improved reproducibility among sequencing runs and studies, and compatibility with lower-quality DNA [@bose2018], making it particularly well suited for conservation applications and non-model taxa. Together, these advantages demonstrate that target-capture datasets can serve as a powerful resource for population genomic analyses while also leveraging data originally generated for phylogenomic studies.

# Applicability to this Workflow 

In this study, we demonstrate a reproducible workflow for generating and analyzing SNP datasets derived from target-capture sequencing data in a conservation genomics framework. Using *H. verticillatus* and its relatives as a case study, we evaluate genetic diversity and population structure across multiple previously known and newly discovered populations, and compare our results with previous conservation genetic analyses based on microsatellites. By providing a transparent pipeline that can be applied to any species or target-capture probe set, this study illustrates how target-capture datasets can be leveraged to address conservation genomic questions and inform management decisions for rare and endangered species.


# Sampling

Leaf tissue was sampled from all known wild populations of *Helianthus verticillatus*: Alabama1, Alabama2, Georgia, McNairy (Tennessee), Pinson (Tennessee), w-Marshall (Mississippi), and Virginia. Seeds were obtained from the two accessions of *H. verticillatus* maintained by the United States Department of Agriculture–Agricultural Research Service (USDA-ARS). Leaf tissue was sampled from all known populations of *Helianthus* sp. in Mississippi: Belzonia, Benton, Greenwood, Grenada, Marshall, and Tchula. Additionally, seeds were obtained from eight USDA-ARS accessions of *H. grosseserratus* populations, spanning its geographic range: Illinois, Iowa, Kansas, Mississippi, Nebraska, North Dakota, Oklahoma, and Wisconsin. Some downstream analyses require the inclusion of an outgroup, so a single individual of *Helianthus schweinitzii* Torr. & A.Gray was included.

The sampling map below features the *H. verticillatus* population locations at their county centroids, since the species is federally endangered and exact locality information cannot be shared.
...
Here are some tips for utilizing this workflow:

---

**Programming Languages**

Throughout this guide, code chunks are labeled according to the programming language in which they should be executed. Code labeled with `bash` are intended to be run from a Linux command line or high-performance computing (HPC) environment, whereas code labeled with `r` should be executed within R or RStudio. This guide was developed using R version 4.3.2. Distinguishing between these environments is intended to provide readers with clear guidance on where each step of the workflow should be performed.
...
│
├── OutgroupDataset/
│   ├── Outgroup_Sample1_R1.fastq.gz
│   ├── Outgroup_Sample1_R2.fastq.gz
...
Use `FastQC` to generate per-sample quality reports and `MultiQC` to summarize results across all samples.
...
```bash
# bash
fastqc /Project/RawReads/*.fastq.gz -o /Project/FastQCRaw/
```
...
```bash
# bash
multiqc /Project/FastQCRaw/ -o /Project/FastQCRaw/
```
...
Use `FastQC` on the trimmed reads to generate per-sample quality reports and `MultiQC` to summarize results across all samples.
...
```bash
# bash
fastqc /Project/Trimming/*.tp.fastq.gz -o /Project/FastQCTrimmed/
```
...
```bash
# bash
multiqc /Project/FastQCTrimmed/ -o /Project/FastQCTrimmed/
```
...
Compare the pre- and post-trimming `MultiQC` reports. In the post-trimming report, there should be:
...
- Sorted binary alignment files (`*.bam`)
- Sorted alignment files (`*.sam`)
- BAM index files (`*.bai`)
...
Before reads are aligned, index the reference genome. This creates files that allow the alignment software to efficiently search the genome.
...
After aligning reads to the reference genome, identify variant sites across all samples and then filter this dataset to generate filtered, high-quality SNPs ready for downstream analysis. 
...
* [`Bowtie2`](https://bowtie-bio.sourceforge.net/bowtie2/manual.shtml) [@langmead2012] – identifies variants across the alignment and filters the resulting file
...
* [`dplyr`](https://dplyr.tidyverse.org/) [@wickham2023] – R package that assists with data wrangling and distance-bin assignment
* [`ggplot2`](https://ggplot2.tidyverse.org/) [@wickham2016] – R package for creating graphics 
...
| `-Ou` | Outputs an uncompressed BCF stream suitable for the `call` command |
...
3. Retain only biallelic SNPs — removes invariant sites and indels
...
Extract all samples belonging to a single population from the VCF:
...
Convert the population-specfic VCF to PLINK format:
...
Read the `plink.ld` table into `R`:
...
Create custom distance bin breaks for the r² plot:
...
To summarize the SNP statistics for any stage in the filtering process:
...
The process for generating the outgroup dataset is similar to the workflow described above, with modifications to the SNP filtering steps.
...
* [`MultiQC`](https://docs.seqera.io/multiqc/) [@ewels2016] – summarizes quality-control reports across samples
* [`Trimmomatic`](http://www.usadellab.org/cms/?page=trimmomatic) [@bolger2014] – removes adapters and low-quality bases from raw sequencing reads
* [`Bowtie2`](https://bowtie-bio.sourceforge.net/bowtie2/manual.shtml) [@langmead2012] – indexes the reference genome, aligns paired-end reads to the reference genome, identifies variants across the alignment, and filters the resulting file
* [`SAMtools`](https://www.htslib.org/doc/samtools.html) [@li2009; @danecek2021] – converts, sorts, indexes, and processes SAM/BAM alignment files
* [`PLINK`](https://www.cog-genomics.org/plink/1.9/) [@chang2015] – performs LD pruning
...
For the outgroup dataset, LD filtering was applied but MAF filtering was not, allowing rare variants to be retained while reducing redundancy among highly correlated SNPs.
...
- `Outgroup_Sample1_R1.fastq.gz` — forward reads
- `Outgroup_Sample1_R2.fastq.gz` — reverse reads
...
The following filess are written to `/Project/OutgroupDataset/`:

- `Outgroup_SNPdataset_raw.vcf.gz` — raw SNP calls
- `Outgroup_SNPdataset_FinalFiltered.vcf.gz` — filtered SNP calls
...
Create a folder for the outgroup dataset (`/Project/OutgroupDataset/`) and move into it. Copy the raw outgroup sequencing reads (`.fastq.gz`) into the folder:
...
```bash
# bash
fastqc /Project/OutgroupDataset/*.fastq.gz -o /Project/OutgroupDataset/
```
...
```bash
# bash
multiqc /Project/OutgroupDataset/ -o /Project/OutgroupDataset/
```
...
Identify variants using the `BCFtools` functions `mpileup` and `call` by comparing aligned reads in all BAM files (including the outgroup) to the reference genome.
...
Identify variants using the `BCFtools` functions `mpileup` and `call` by comparing aligned reads in the BAM files belonging to a single species to the reference genome.

```bash
# bash
bcftools mpileup -Ou -f /Project/ReferenceGenome/ReferenceGenome.fasta -b POPULATION1_SAMPLE1.bam,POPULATION1_SAMPLE2.bam,POPULATION1_SAMPLE3.bam | bcftools call -mv -Oz -o Species1_SNPDataset_raw.vcf.gz
```
...
Repeat this workflow for each species to create all the species-separated SNP datasets.
...
This file contains a list of all samples and their associated species assignments. Do not include the outgroup.
...
This file contains a list of all samples and their associated population assignments. Do not add the outgroup.
...
Principal components analysis (PCA) is a dimensionality reduction method used to visualize genetic relationships among samples. By summarizing genome-wide variation into a small number principal components, PCA assists with the identification of clustering patterns that reflect population structure and species boundaries.
...
* [`SNPRelate`](https://bioconductor.org/packages/SNPRelate/) [@zheng2012] – R package that converts VCF files to GDS format and performs principal component analysis on genome-wide SNP datasets
* [`dplyr`](https://dplyr.tidyverse.org/) [@wickham2023] – R package that assists with data wrangling and distance-bin assignment
...
* [`plotADMIXTURE.r`](https://github.com/speciationgenomics/) – R script that formats and plots ADMIXTURE ancestry estimates for publication-quality visualization
...
Neighbor-joining (NJ) trees provide a graphical representation of genetic relationships among samples based on pairwise genetic distances [@saitou1987]. Unlike PCA and ADMIXTURE, neighbor-joining trees display genetic relationships as a branching network, making it easier to visualize which individuals and populations are most genetically similar.

In this workflow, the NJ tree is constructed using the `neighbor` function from the `PHYLIP` program [@felsenstein1989].
...
Open the `.dist` file generated by `PLINK` and convert it to PHYLIP format following these [guidelines](https://condor.depaul.edu/waguirre/phylip_making_trees.pdf) created by Windsor Aguirre.
...
The `outtree` file is used for visualization. Open `FigTree` and load the `outtree` file.
...
Samples that cluster together on the tree are more genetically similar than samples separated by longer branch lengths.
...
- Admixed individuals occupying positions between major clusters
...
* [`vcfR`](https://cran.r-project.org/package=vcfR) [@knaus2017] – R package that imports VCF files into `R` and converts variant data into formats compatible with population genetic analyses
* [`dartR.popgen`](https://cran.r-project.org/package=dartR.popgen) [@gruber2018; @mijangos2022] – R package calculates population genetic summary statistics
...
- `Species1_FST.csv` — example pairwise F<sub>ST</sub> estimates among populations
- `Species1_Heterozygosity.csv` — example observed heterozygosity, expected heterozygosity, and inbreeding statistics
...
```r
# r
install.packges("vcfR")
install.packges("dartR.popgen")

library(vcfR)
library(dartR.popgen)
```

Read the species-specific VCF into `R` and convert it to a `genlight` object:
...
Read the population metadata file into `R`:
...
Previous analyses (PCA, ADMIXTURE, neighbor-joining trees, and genetic diversity statistics) characterize patterns of genetic structure within the dataset. However, these approaches do not explicitly evaluate historical gene flow among populations.

To investigate potential hybridization and introgression, several methods can be employed that test for gene flow using different statistical frameworks. Together, these analyses can identify historical migration events, quantify admixture, and evaluate hypotheses regarding population relationships.
...
In this workflow, `TreeMix` is applied to the Outgroup dataset.
...
* [`vcfR`](https://cran.r-project.org/package=vcfR) [@knaus2017] – R package that imports VCF files into `R` and converts variant data into formats compatible with population genetic analyses
* [`dartR.popgen`](https://cran.r-project.org/package=dartR.popgen) [@gruber2018; @mijangos2022] – R package that converts population genomic datasets into `TreeMix`-compatible input formats
* [`RColorBrewer`](https://cran.r-project.org/package=RColorBrewer) [@Neuwirth2022] – R package that provides color palettes for visualizing `TreeMix` results
* [`R.utils`](https://cran.r-project.org/package=R.utils) [@bengtsson2023] – R package that provides utility functions required for processing and plotting `TreeMix` outputs
...
Convert the VCF to TreeMix format using R. Load in the required packages:
...
`TreeMix` will run across a range of migration edge values (*m* = 0–10). Multiple replicates are performed for each value of *m* to assess model stability.
...
Determine the optimal number of migration edges with `OptM`. Open an `R` environment and load the required packages:
...
Because *f3*-statistics do not require information about ancestral and derived alleles, an outgroup is not required for this analysis. The Ingroup dataset is used for this analysis.
...
Convert the VCF to EIGENSTRAT format. Open an `R` environment and load the required packages:
